...
fn main() {
    unsafe {
        ...
        let shader = create_shader(
            read_shader("shaders/mat-gpu.comp.glsl")
        );
        ...
        let a =
            alloc_hardware_buffer(ELEMENT_COUNT as u32 * 64);
        let a_mapped =
            map_hardware_buffer(a, ELEMENT_COUNT as i32 * 64);
        fill_random::<f32>(a_mapped, ELEMENT_COUNT * 16, GRND_URANDOM);
        ...
        let b =
            alloc_hardware_buffer(ELEMENT_COUNT as u32 * 64);
        let b_mapped =
            map_hardware_buffer(b, ELEMENT_COUNT as i32 * 64);
        fill_random::<f32>(b_mapped, ELEMENT_COUNT * 16, GRND_URANDOM);
